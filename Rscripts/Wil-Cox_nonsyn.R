  ...
  fingernotes <-c("5UTR_nonsyn","Gag_nonsyn","Pol_nonsyn","Vif_nonsyn","Vpr_nonsyn","Env_nonsyn","Nef_nonsyn")
  data1<- OverviewDFZanini[1:634,1:10] #5UTR
  data2<- OverviewDFZanini[790:2092,1:10] #Gag
  data3<- OverviewDFZanini[2358:5040,1:10] #pol
  data4<- OverviewDFZanini[5041:5557,1:10] #vif
  data5<- OverviewDFZanini[5559:5850,1:10] #vpr
  data6<- OverviewDFZanini[6225:8795,1:10] #env
  data7<- OverviewDFZanini[8797:9168,1:10] #nef
  ...
  array1 = get(paste("data",j,sep = ""))$colMeansTsZanini[get(paste("data",j,sep = ""))$WTnt =="a" & get(paste("data",j,sep = ""))$TypeOfSite == 'nonsyn' & get(paste("data",j,sep = ""))$makesCpG == 1]
  array2 = get(paste("data",j,sep = ""))$colMeansTsZanini[get(paste("data",j,sep = ""))$WTnt =="a" & get(paste("data",j,sep = ""))$TypeOfSite == 'nonsyn' & get(paste("data",j,sep = ""))$makesCpG == 0]
  
  print("For a: Comparing makes CpG with noCpG (nonsyn). Wilcox test greater: red/blue")
  print(wilcox.test(array1, array2, alternative='greater'))
  pVals = c(pVals,format(wilcox.test(array1, array2, alternative='greater')$p.value, nsmall = 6))
  print(pVals)
  
  
  
  
  array5 = get(paste("data",j,sep = ""))$colMeansTsZanini[get(paste("data",j,sep = ""))$WTnt =="t" & get(paste("data",j,sep = ""))$TypeOfSite == 'nonsyn' & get(paste("data",j,sep = ""))$makesCpG == 1]
  print(array5)
  array6 = get(paste("data",j,sep = ""))$colMeansTsZanini[get(paste("data",j,sep = ""))$WTnt =="t" & get(paste("data",j,sep = ""))$TypeOfSite == 'nonsyn' & get(paste("data",j,sep = ""))$makesCpG == 0]
  print(array6)

  
  print("For t: Comparing makes CpG with noCpG (nonsyn). Wilcox test greater: red/blue")
  print(wilcox.test(array5, array6, alternative='greater'))
  pVals = c(pVals,format(wilcox.test(array5, array6, alternative='greater')$p.value, nsmall = 6))
...
print(Pvalues)
  ...
  title(main = fingernotes[j], family = "Times", adj = 0.5, cex.main= 2)
  abline(v = 100/5)
  abline(v = 2*100/3)
