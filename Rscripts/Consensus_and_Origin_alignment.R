...
Aligned_Geneious<-read.fasta(file = "Data/AlignedGeneious.fasta", seqtype = "DNA", as.string = FALSE, forceDNAtolower = TRUE, set.attributes = TRUE, legacy.mode = TRUE, seqonly = FALSE, strip.desc = FALSE, bfa = FALSE, sizeof.longlong = .Machine$sizeof.longlong, endian = .Platform$endian, apply.mask = TRUE)
Realnames<-vector()
...
Aligned_Geneious$`HIV_Reference_(Consensus_B)`
addressdeletion
...
for (i in 1:length(ZaniniFiles)){ #rows 72
  ...
  for (k in 1:length(Aligned_Geneious)){#70
    ...
    if (ZaniniModifiedName == Realnames_2[k]){
      ...
      UnalignedCount$MajNt<-apply(UnalignedCount[,1:4],1,function(x){ if (max(x)==0) NA
      ...
      write.csv(AlignedCount, file = paste("/Users/Calmair/HIV/Data/Modified_Sequences_Aligned2_Consensus/modified",gsub("/","_",ZaniniFiles[i] ),sep = "_"),row.names = FALSE)
    }
