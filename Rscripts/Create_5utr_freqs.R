...
consensusB<-consensusfasta$CONSENSUS_B[551:873] #changed consensus B to 5utr only
#* Transition function*
...
ZaniniFiles<-list.files("Data/Modified_Sequences_Aligned2_Consensus/",recursive = TRUE,pattern="tsv")
...
for (i in 1:length(ZaniniFiles)){
  print(i)
  print(ZaniniFiles[i])
  SeqData<-read.csv(paste("Data/Modified_Sequences_Aligned2_Consensus/",ZaniniFiles[1],sep=""),sep=",", stringsAsFactors = FALSE)
  colnames(SeqData)[which(names(SeqData) == "X..A")] <- "A"
  SeqData<-SeqData[551:873,]
  ...
  for (k in 1:length(SeqData$consensusB)){#for each site in the sequence
    ...
    MutNum<- SeqData [k,which(c("a","c","g","t")==SeqData$transition[k])] #NEED CLARIFICATION
...
for (i in 1:length(freqPatTs_Zanini)) hist(freqPatTs_Zanini[,i],main=paste("Site",i+550,"WT:" ,SeqData$MajNt[i]),xlim=c(0,.01),ylim = c(0,10),breaks=seq(0,1,by=0.0001),xlab = "Frequency",ylab="Number of Patients",col=2)
dev.off()
